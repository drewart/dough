...
func NewTabbedPanels() *TabbedPanels {
	...
	s.SetBackgroundColor(Styles.PrimaryTextColor)
	s.SetTextColor(Styles.InverseTextColor)
...
// and spaces.
func (t *TabbedPanels) AddTab(name, label string, item tview.Primitive) {
	t.Lock()
...
func (t *TabbedPanels) updateTabLabels() {
	...
	if t.switcherVertical {
		...
		if t.switcherHeight > 0 {
			...
			reqLines = len(tview.WordWrap(t.Switcher.GetText(true), t.width))
			if reqLines < 1 {
				reqLines = 1
			}
		}
...
func (t *TabbedPanels) InputHandler() func(event *tcell.EventKey, setFocus func(p tview.Primitive)) {
...
func (t *TabbedPanels) MouseHandler() func(action tview.MouseAction, event *tcell.EventMouse, setFocus func(p tview.Primitive)) (consumed bool, capture tview.Primitive) {
